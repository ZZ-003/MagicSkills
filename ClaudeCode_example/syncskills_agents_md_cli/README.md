...
执行 `magicskills addskills claudecode --skill-list c_2_ast brand-guidelines --agent-md-path ./AGENTS.md`

### 生成 AGETNS.md

执行 `magicskills syncskills claudecode --mode cli_description --output ./AGENTS.md -y` 指定输出AGENTS.md路径，不指定时候就默认用`addskills`指定的`--agent-md-path ./AGENTS.md`
此时AGENTS.md会出现如下内容
