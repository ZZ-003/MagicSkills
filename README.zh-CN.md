...
- `SkillsRegistry`：描述多个命名 `Skills` 集合如何被注册、加载和持久化
...
5. 最后，这个命名的 `Skills` 集合会被注册到 `SkillsRegistry` 中，可选地持久化，并同步到 `AGENTS.md`
...
`SkillsRegistry` 层解决的是如何保存和恢复多个命名的 skills 集合。
...
所以本质上，Registry 层是 MagicSkills 的项目级配置中心。`Skill` 定义单体，`Skills` 组织工作集，而 `SkillsRegistry` 让这些集合能跨不同运行周期持续存在。
...
| `changetooldescription` | 修改集合的 `tool_description` 元数据 | 更新集合描述，供后续查询和集成使用 |
