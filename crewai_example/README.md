...
```bash
uv run --with "crewai[litellm]" --with crewai-tools --with python-dotenv python crewai_example/model.py --scenario all
```

可选：只跑单个场景

```bash
uv run --with "crewai[litellm]" --with crewai-tools --with python-dotenv python crewai_example/model.py --scenario read
uv run --with "crewai[litellm]" --with crewai-tools --with python-dotenv python crewai_example/model.py --scenario exec
```
