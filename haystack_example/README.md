...
## 1. 目标
本示例演示如何把 MagicSkills 的 skill_tool 接入 Haystack agent，并生成两份完整日志：

- log1.json：不停读文档场景（偏知识学习）
- log2.json：执行场景（把 C 代码转换为 AST）

日志会保留 agent 每一步输出（包含中间 tool 调用与最终回答），格式参考 haystack_example/log1.json 与 haystack_example/log2.json。
...
```bash
magicskills addskills haystack_agent1_skills --skill-list c_2_ast pdf --agent-md-path ./AGENTS.md
magicskills addskills haystack_agent2_skills --skill-list c_2_ast docx --agent-md-path ./AGENTS.md
```

- **haystack_agent1_skills**：`c_2_ast` + `pdf`，用于 log1 场景（知识阅读 agent）
- **haystack_agent2_skills**：`c_2_ast` + `docx`，用于 log2 场景（代码执行 agent）


## 5. 生成 AGENTS.md

```bash
magicskills syncskills haystack_agent1_skills --output ./AGENTS.md -y
```
...
```bash
python haystack_example/model.py --scenario read
python haystack_example/model.py --scenario exec
```
