...
```bash
uv run --with haystack-ai --with python-dotenv python haystack_example/model.py --scenario all
```

可选：只跑单个场景

```bash
uv run --with haystack-ai --with python-dotenv python haystack_example/model.py --scenario read
uv run --with haystack-ai --with python-dotenv python haystack_example/model.py --scenario exec
```
