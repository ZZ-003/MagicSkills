...
## 1. 目标
本示例演示如何把 MagicSkills 的 skill_tool 接入 LangGraph agent，并生成两份完整日志：
...
```bash
magicskills addskills langgraph_agent1_skills --skill-list c_2_ast pdf --agent-md-path ./AGENTS.md
magicskills addskills langgraph_agent2_skills --skill-list c_2_ast docx --agent-md-path ./AGENTS.md
```
...
```bash
magicskills syncskills langgraph_agent1_skills --output ./AGENTS.md -y
```
...
```bash
python langgraph_example/model.py --scenario read
python langgraph_example/model.py --scenario exec
```
